...
from app.models import Book, Chapter, ContentBlock, DocumentMember
from app.schemas.books import ContentRead, ContentUpdate

router = APIRouter(prefix="/chapters", tags=["chapters"])


async def accessible_chapter(chapter_id: int, user_id: int, session: DbSession) -> Chapter:
    chapter = await session.scalar(
        select(Chapter).join(Book).where(
            ...
            or_(
                ...
                Chapter.id.in_(
                    select(DocumentMember.chapter_id).where(DocumentMember.user_id == user_id)
                ),
            ),
        )
    )
...
async def get_content(chapter_id: int, user: CurrentUser, session: DbSession):
    await accessible_chapter(chapter_id, user.id, session)
